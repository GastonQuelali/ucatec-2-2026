...
flowchart LR
    subgraph MANUAL["Proceso manual"]
        M1["Se recibe un pedido"] --> M2["Alguien lo escribe a mano"] --> M3["Se envía mail a mano"]
    end
    subgraph AUTO["Proceso automatizado"]
        A1["Se recibe un pedido"] --> A2["El sistema lo registra solo"] --> A3["El mail se envía solo"]
    end
```

*Figura: Proceso manual vs proceso automatizado*
...
```mermaid
flowchart TD
    PREG["¿El proceso es repetitivo y usa datos?"]
    PREG -- "Sí" --> REP["¿Sigue reglas fijas?"]
    PREG -- "No" --> NO["No conviene<br/>(tarea creativa o excepcional)"]
    REP -- "Sí" --> SÍ["AUTOMATIZAR"]
    REP -- "No" --> NO2["Automatización parcial o manual"]
```
...
```mermaid
flowchart LR
    BD["Tabla Libros<br/>(Stock < 5)"] --> CHK["Detector<br/>(¿cambió el stock?)"]
    CHK --> NOT["Notificación<br/>(mail / mensaje)"]
    NOT --> ENC["Encargado de compras<br/>(recibe el aviso)"]
```
...
```mermaid
flowchart LR
    T["Temporizador<br/>(lunes 8:00)"] --> Q["Consulta<br/>ventas de la semana"]
    Q --> R["Generar reporte<br/>(PDF / hoja)"]
    R --> E["Enviar por mail<br/>a la gerencia"]
```

*Figura: Flujo de generación automática de reportes semanales*
...
```mermaid
flowchart LR
    PED["Cliente hace un pedido"] --> REG["Se crea el pedido<br/>en la tabla Pedidos"]
    REG --> STK["Se actualiza el stock<br/>en la tabla Productos"]
    STK --> FIN["Pedido listo"]
```

*Figura: Flujo de actualización automática de registros al recibir un pedido*
...
```mermaid
flowchart LR
    APP1["Aplicación A<br/>(fuente)"] --> TRIG["Trigger<br/>(desencadenador)"]
    TRIG --> FLUJO["Flujo de trabajo<br/>(pasos y filtros)"]
    FLUJO --> ACT["Acción/es<br/>(Aplicación B, C...)"]
```

*Figura: Estructura general de una automatización con conectores*
...
```mermaid
flowchart TD
    SUB["¿Qué plataforma elegir?"]
    SUB --> MS["¿La empresa usa Microsoft?<br/>(Outlook, Teams, Office)"]
    MS -- "Sí" --> PA["Power Automate"]
    MS -- "No" --> ZAP["Zapier"]
    SUB --> GRAT["¿Busca el plan gratuito más simple?"]
    GRAT --> ZAP2["Zapier (comenzar rápido)"]
```
...
```mermaid
flowchart TD
    P1["1. Elegir el proceso"] --> P2["2. Identificar el trigger"]
    P2 --> P3["3. Definir las acciones"]
    P3 --> P4["4. Agregar condiciones/filtros"]
    P4 --> P5["5. Probar el flujo"]
    P5 --> P6["6. Monitorear y ajustar"]
```

*Figura: Pasos para diseñar un flujo de trabajo automatizado*
