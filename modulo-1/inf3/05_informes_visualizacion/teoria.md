...
```mermaid
flowchart LR
    BD["Base de datos"] --> CON["Consulta<br/>(filtra y agrupa)"]
    CON --> INF["Informe<br/>(presenta y resume)"]
    INF --> DASH["Dashboard<br/>(visualiza KPIs)"]
    DASH --> DEC["Decisión"]
```

*Figura: El camino desde los datos hasta la decisión*
...
```mermaid
flowchart TD
    A["Encabezado del informe<br/>(título, logotipo, fechas)"]
    B["Encabezado de página<br/>(nº de página, logos)"]
    C["Cuerpo / detalle<br/>(los registros)"]
    D["Encabezado y pie de grupo<br/>(totales por grupo)"]
    E["Pie de página<br/>(nº de página)"]
    F["Resumen del informe<br/>(totales generales)"]
```

*Figura: Estructura de un informe en Access*
...
```mermaid
flowchart TD
    DATO["Datos crudos<br/>(muchas filas)"] --> VIS["Visualización<br/>(gráfico)"]
    VIS --> LECT["Lectura rápida<br/>(tendencias, comparaciones)"]
```

*Figura: La visualización convierte filas de datos en lectura rápida*
...
```mermaid
flowchart LR
    subgraph DASH["Dashboard de ventas"]
        K1["KPI 1<br/>Ventas del mes"] --> G1["Gráfico líneas<br/>Evolución"]
        K2["KPI 2<br/>Ticket promedio"] --> G2["Gráfico barras<br/>Por producto"]
        K3["KPI 3<br/>Clientes nuevos"] --> G3["Tabla resumen"]
    end
```

*Figura: Un dashboard combina KPIs y gráficos*
...
```mermaid
flowchart LR
    IMP["Importar datos<br/>(Excel/Access/SQL)"] --> MOD["Modelar<br/>(relaciones, cálculos)"]
    MOD --> VIS["Visualizar<br/>(gráficos)"]
    VIS --> PUB["Publicar y compartir"]
```
...
```mermaid
flowchart LR
    D1["1. Pregunta<br/>¿qué querés saber?"] --> D2["2. Datos<br/>¿de dónde salen?"]
    D2 --> D3["3. Análisis<br/>consulta + informe"]
    D3 --> D4["4. Visualización<br/>dashboard"]
    D4 --> D5["5. Decisión<br/>acción informada"]
    D5 --> D6["6. Medición<br/>¿funcionó?"]
    D6 --> D1
```

*Figura: El ciclo completo de la toma de decisiones basada en datos*
