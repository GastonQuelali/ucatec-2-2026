...
```mermaid
flowchart TD
    A["¿Necesitás una BD?"] --> B{"¿Múltiples usuarios\nremotos?"}
    B -->|Sí| C{"¿Necesitás SQL\ny escalabilidad?"}
    C -->|Sí| MYSQL["MySQL / PostgreSQL<br/>(servidor)"]
    C -->|No| CLOUD["Google Tables / Airtable<br/>(nube)"]
    B -->|No| D{"¿Es para aprender\no una PYME?"}
    D -->|Sí| ACCESS["Microsoft Access<br/>(escritorio)"]
    D -->|No| MYSQL
```

*Figura: Árbol de decisión para elegir una herramienta SGBD*
...
```mermaid
flowchart TB
    subgraph VISTA_DATOS["Vista Hoja de datos"]
        A1["ID"] --- A2["Nombre"] --- A3["Email"]
        A4["1"] --- A5["Ana"] --- A6["ana@email.com"]
        A7["2"] --- A8["Luis"] --- A9["luis@email.com"]
    end
    subgraph VISTA_DISENO["Vista Diseño"]
        B1["Campo: ID<br/>Tipo: Autonumérico<br/>PK: Sí"]
        B2["Campo: Nombre<br/>Tipo: Texto corto<br/>PK: No"]
        B3["Campo: Email<br/>Tipo: Texto largo<br/>PK: No"]
    end
```
...
```mermaid
flowchart LR
    A["1. Diseñar<br/>(campos y tipos)"] --> B["2. Crear tabla<br/>(Vista Diseño)"]
    B --> C["3. Insertar datos<br/>(Vista Hoja de datos)"]
    C --> D["4. Modificar<br/>(agregar/cambiar campos)"]
    D --> E["5. Consultar<br/>(filtros y consultas)"]
    E --> F["6. Eliminar / archivar<br/>(si ya no se usa)"]
```

*Figura: Ciclo de vida de una tabla en Access*
...
```mermaid
mindmap
  root((Tipos de datos<br/>en Access))
    Texto
      Texto corto
      Texto largo
    Numérico
...
*Figura: Clasificación de los tipos de datos en Access*
...
```mermaid
flowchart TD
    A["1. Nombre claro y singular<br/>(ej. Cliente, no Clientes/Clientes list)"]
    B["2. Campo ID autonumérico<br/>como clave primaria"]
    C["3. Cada dato en su propio campo<br/>(separar Nombre y Apellido)"]
    D["4. Elegir el tipo de dato correcto<br/>para cada campo"]
    E["5. No repetir datos<br/>(un dato una sola vez)"]
    F["6. Definir propiedades:<br/>tamaño, formato, validación"]
    G["7. Pensar en los datos futuros<br/>(¿qué pasa cuando crezcan?)"]
    H["8. Definir relaciones con otras tablas<br/>(claves foráneas)"]
    A --> B --> C --> D --> E --> F --> G --> H
```

*Figura: Checklist de buenas prácticas para diseñar una tabla*
...
```mermaid
flowchart TB
    subgraph MAL["❌ Diseño incorrecto (datos repetidos)"]
        M1["Pedido 1 — Ana Pérez — La Paz"]
        M2["Pedido 2 — Ana Pérez — La Paz"]
    end
    subgraph BIEN["✅ Diseño correcto (relaciones)"]
        C1["Clientes: ID 1 → Ana Pérez → La Paz"]
        P1["Pedido 1 → Cliente_ID 1"]
        P2["Pedido 2 → Cliente_ID 1"]
    end
```

*Figura: Normalización básica — eliminar datos repetidos usando claves*
