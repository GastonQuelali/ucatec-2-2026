...
```mermaid
flowchart LR
    U["Usuario"] --> AP["Aplicación<br/>(formulario, página web)"]
    AP --> SQL["Instrucción SQL"]
    SQL --> SGBD["SGBD<br/>(base de datos)"]
    SGBD --> R["Resultado<br/>(datos)"]
    R --> U
```

*Figura: SQL es el puente entre la aplicación y la base de datos*
...
```mermaid
flowchart LR
    subgraph ACCESO_VISUAL["Interfaz visual (Access)"]
        A1["Clic en menú"] --> A2["Diseñar consulta en cuadrícula"] --> A3["Ejecutar"]
    end
    subgraph ACCESO_SQL["Con SQL"]
        B1["Escribir sentencia"] --> B2["Ejecutar"]
    end
```

*Figura: Dos formas de consultar: por interfaz o por SQL*
...
```mermaid
flowchart TD
    SQL["Comandos DML básicos"] --> SEL["SELECT<br/>Leer datos"]
    SQL --> INS["INSERT<br/>Agregar datos"]
    SQL --> UPD["UPDATE<br/>Modificar datos"]
    SQL --> DEL["DELETE<br/>Eliminar datos"]
```
...
```mermaid
flowchart LR
    S["SELECT campo1, campo2"] --> F["FROM tabla"]
    F --> W["WHERE condicion"]
    W --> O["ORDER BY campo"]
```

*Figura: Orden de las cláusulas en una sentencia SELECT*
...
```mermaid
flowchart LR
    USR["Usuario"] --> F["Formulario<br/>(interfaz)"]
    F --> VAL["Validación<br/>(tipos, obligatorios)"]
    VAL --> SQL1["Instrucción SQL<br/>(INSERT / UPDATE)"]
    SQL1 --> BD["Base de datos"]
```

*Figura: Un formulario convierte la entrada del usuario en instrucciones SQL*
...
```mermaid
flowchart TD
    HERR["Herramientas de captura de datos"]
    HERR --> ACC["Microsoft Access<br/>(formularios conectados a BD local)"]
    HERR --> GF["Google Forms<br/>(encuestas → Google Sheets)"]
    HERR --> AP["AppSheet<br/>(apps móviles sin código sobre datos)"]
```

*Figura: Tres herramientas de captura de datos*
